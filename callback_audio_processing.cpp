 ...
 * Sidechain Compressor programmed by Rob Salino (c) 2020
 * 
 *
 */
...
double ratio[6] = {1.0, 2.0, 4.0, 8.0, 20.0, 25.0};
...
//LEFT CHANNEL WILL BE MAIN, RIGHT CHANNEL WILL BE "KEY" OR THE SIDECHAIN
//RAT is to cycle through the ratio values assigned to the array ratio
	...
	}//add the drum track or silence (not same as bypassing, still compresses using key signal)
    ...
    //assigning the pots to threshold, attack, release time
	threshold = 48.0*(multicore_data->audioproj_fin_pot_hadc0)-48.0;
	attack_time = (multicore_data->audioproj_fin_pot_hadc1)*0.2;
	release_time = (multicore_data->audioproj_fin_pot_hadc2)*0.2;
    //attack and release based on time constant, sample rate, and where each pot is turned to:
	AT = exp(TC/(attack_time * AUDIO_SAMPLE_RATE));
	RT = exp(TC/(release_time * AUDIO_SAMPLE_RATE));
	// Otherwise, perform our C-based block processing here!
	for (int i = 0; i < AUDIO_BLOCK_SIZE; i++) {
		//left channel is "main" audio
		float rect_LEFT = (pow(audiochannel_0_left_in[i], 2));
		//right channel is the "key" or sidechain
		float rect_RIGHT = (pow(audiochannel_0_right_in[i], 2));
        ...
        //if rectified left channel is greater than previous envelope value, make its envelope based on attack time and the envelope of the right channel, or key signal
		if(rect_LEFT > env_prev)
		{
			env = AT*(env_prev)+(1-AT)*rect_RIGHT;
		}
		else //else the envelope is based on the release time and the envelope of the right channel, or key signal
		{
			env = RT*(env_prev)+(1-RT)*rect_RIGHT;
		} //if env is less than 0, set to 0
		if(env < 0)
		{
			env = 0;
		}
        //apply the new envelope value
		env_prev = env;
        //take square root of envelope
		env = sqrt(env);

		if(env <= 0) //if env less than or equal 0, set detector to -96 (no detection)
		{
			detector = -96;
		}
		else //else detector is the envelope in dB
		{
			detector = 20.0*log10(env);
		}

		if(detector <= threshold) //if detector is less than or equal to threshold (set by pot), set gain to 0
		{
			Gain = 0;
		}
		else //else, make the gain essentially based on the slope created by the detector value over the ratio (this is why higher ratio makes a more noticeable compression)
		{
			Gain = threshold + ((detector - threshold)/ratio[RAT]) - detector;
		}
		if(PASS == 0 && ADD == 1)//if not bypassed and also want to add key/sidechain signal:
		{
			audiochannel_0_left_out[i] = (pow(10,(Gain/20))*audiochannel_0_left_in[i]) + audiochannel_0_right_in[i];
			audiochannel_0_right_out[i] = (pow(10,(Gain/20))*audiochannel_0_left_in[i]) + audiochannel_0_right_in[i];
		}
		else if(PASS == 0 && ADD == 0)//if not bypassed but do NOT want to add key/sidechain signal:
		{
			audiochannel_0_left_out[i] = (pow(10,(Gain/20))*audiochannel_0_left_in[i]);
			audiochannel_0_right_out[i] = (pow(10,(Gain/20))*audiochannel_0_left_in[i]);
		}
		else if(PASS == 1)//bypass
		{
			audiochannel_0_left_out[i] = audiochannel_0_left_in[i];
